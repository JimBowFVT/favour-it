...
function Avatar({ person, large = false }) {
  return person?.avatar_url ? (
    <img
      className={`public-profile-avatar ${large ? 'large' : ''}`}
      src={person.avatar_url}
      alt=""
    />
  ) : (
    <div className={`public-profile-avatar ${large ? 'large' : ''}`}>
...
export default function PublicProfile({
  ...
  const report = async () => {
    ...
    setError('');
    try {
      await reportUser(profile.id, reason.trim(), details.trim());
  ...
  const renderConnectionActions = () => {
    ...
    if (friendStatus === 'friends') {
      connectionAction = (
        <span className="connection-state">Friends ✓</span>
      );
      ...
      connectionAction = (
        <>
          <button
            ...
            onClick={() => action(() => respondFriendRequest(profile.id, 'accept'))}
          >
            Accept
          </button>
          <button
            className="secondary"
            disabled={busy || blocked}
            onClick={() => action(() => respondFriendRequest(profile.id, 'reject'))}
          >
            Decline
          </button>
        </>
      ...
      connectionAction = (
        <button
          className="primary"
          disabled={busy}
          onClick={() => action(() => sendFriendRequest(profile.id))}
        >
          Connect
        </button>
      );
    ...
    return (
      <>
        ...
        <div className="public-profile-menu-wrap">
          <button
          ...
          {menuOpen && (
            <div className="public-profile-menu">
              {profile.blocked_by_me ? (
                <button
                  disabled={busy}
                  onClick={() => action(() => unblockUser(profile.id))}
                >
                  Unblock
                </button>
              ) : (
                <button
                  ...
                  onClick={() => {
                    if (
                      window.confirm(
                        `Block @${profile.username}? Your existing conversation will be kept.`,
                      )
                    ) {
          ...
          <>
            <header className="public-profile-header">
              <div className="public-profile-identity">
                ...
                <div>
                  <div className="eyebrow">FAVOURIT MEMBER</div>
                  <h1 id="public-profile-title">
              ...
              <div className="public-profile-header-actions">
            ...
            {profile.blocked_by_me && (
              <div className="public-profile-notice">
                You blocked this member. Your existing conversation and messages
                are still kept. Unblock them from the ••• menu to reconnect.
              </div>
            )}
            {profile.blocked_me && (
              <div className="public-profile-notice">
                This member has blocked you. Some interactions are unavailable.
              </div>
            )}

            <div className="public-profile-stats">
              <div>
                <strong>{Number(stats.rating || 0).toFixed(1)}</strong>
                <span>Rating · {stats.review_count || 0} reviews</span>
              </div>
              <div>
                <strong>{stats.completed_deals || 0}</strong>
                <span>Completed deals</span>
              </div>
              <div>
                <strong>{stats.published_deals || 0}</strong>
            ...
            <section className="public-profile-section">
              <div className="public-profile-section-heading">
                <div>
                  ...
                  <h2>
                    What {String(profile.display_name || profile.username).split(' ')[0]} offers
                  </h2>
                </div>
                <span>{profile.deals?.length || 0}</span>
              </div>
              {profile.deals?.length ? (
                <div className="public-profile-deals">
                  {profile.deals.map((deal) => (
                    <article className="public-deal-card" key={deal.id}>
                      <span className="public-deal-category">{deal.category}</span>
                      <h3>{deal.title}</h3>
                      <p>{deal.description}</p>
                      <footer>
                        <strong>
                          {Number(deal.price_fav || 0).toLocaleString()} FAV
                        </strong>
                        <span>
                          {deal.delivery_days} day{deal.delivery_days === 1 ? '' : 's'}
                        </span>
                      </footer>
                    </article>
                  ))}
                ...
                <div className="public-profile-muted">No public services yet.</div>
              )}
            </section>
            ...
            <div className="public-profile-two-column">
              <section className="public-profile-section">
                ...
                {profile.reviews?.length ? (
                    ...
                    {profile.reviews.map((review) => (
                      <article className="public-review" key={review.id}>
                        ...
                        <div>
                          ...
                          <p>{review.body || 'No written feedback.'}</p>
                  ...
                  <div className="public-profile-muted">No reviews yet.</div>
                )}
              </section>

              <section className="public-profile-section">
                <div className="eyebrow">COMMUNITY</div>
                <h2>Skill communities</h2>
                {joinedGroups.length ? (
                  <div className="public-profile-groups">
                    {joinedGroups.map((group) => (
                      <div className="public-profile-group" key={group.id}>
                  ...
                  <div className="public-profile-muted">
                    Not a member of any public skill community yet.
                  </div>
                )}
              </section>
            </div>
