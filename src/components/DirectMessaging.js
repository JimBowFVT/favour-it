...
export default function DirectMessaging({ session, usernameStatus }) {
  ...
  const refreshConversations = async () => {
    try {
      ...
      setConversations(data.map(c => c.conversation_id === conversationId && open ? { ...c, unread_count: 0 } : c));
  ...
  const refreshMessages = async id => {
    ...
    try {
      ...
      setMessages(current => {
        ...
        if (newLast && newLast !== oldLast && oldLast && data.some(m => m.id === newLast && m.sender_id !== session.user.id)) playMessagePing(audioRef, true);
        return data;
      });
    } catch (_) {}
  };

  useEffect(() => { if (session) refreshConversations(); }, [session, conversationId, open]);
  ...
  useEffect(() => {
    ...
    const setTypingState = (isTyping, timestamp = Date.now()) => {
      if (!active) return;
      if (isTyping) {
        setOtherTyping(true);
        setOtherTypingAt(timestamp);
      } else {
    ...
    const refreshPresence = () => {
      ...
      if (typingEntry) setTypingState(true, Number(typingEntry.typing_at));
      else if (!typingEntry) setTypingState(false);
    };
    ...
    const channel = supabase.channel(`direct-chat-${conversationId}`, {
      ...
      .on('broadcast', { event: 'typing' }, payload => {
        ...
        setTypingState(Boolean(payload.payload?.typing), Number(payload.payload?.typing_at || Date.now()));
      })
    ...
    // Keep the user's own read cursor current while the chat is open.
    // This makes a message become Read even when the recipient was already inside the conversation.
    const readHeartbeat = window.setInterval(() => {
      markCurrentConversationRead(conversationId);
    }, 1500);

    const typingExpiry = window.setInterval(() => {
      if (otherTypingAt && Date.now() - otherTypingAt >= 5000) setTypingState(false);
    }, 1000);
    ...
    return () => {
      ...
      try {
        channel.send({ type: 'broadcast', event: 'typing', payload: { user_id: session.user.id, typing: false, typing_at: 0 } }).catch(() => {});
        channel.untrack();
  ...
  }, [session?.user?.id, conversationId, open]);
  ...
  const leaveConversation = async () => {
    ...
    setSelected(null); setConversationId(null); setMessages([]); setBody(''); setOtherTyping(false); setOtherReadAt(null); refreshConversations();
  };
  ...
  const updateTyping = async value => {
    ...
    if (!channel) return;
    const payload = { user_id: session.user.id, typing: value, typing_at: value ? Date.now() : 0 };
    try {
  ...
  const send = async e => {
    ...
    try {
      ...
      setBody(''); await updateTyping(false); playMessagePing(audioRef, false); refreshConversations();
      ...
      <section className="dm-panel">
        <header className="dm-header">
          <div className="dm-header-identity">
            <span className="dm-header-handle">@{ownUsername}</span><span className="dm-header-title">Messages</span>
        ...
        {!selected ? <>
          ...
          {otherTyping && <div className="dm-typing" role="status" aria-live="polite"><i></i><i></i><i></i><span>{selected.display_name || `@${selected.username}`} is typing</span></div>}
