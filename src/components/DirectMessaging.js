...
export default function DirectMessaging({ session, usernameStatus }) {
  ...
  const [query, setQuery] = useState(''); const [results, setResults] = useState([]); const [conversations, setConversations] = useState([]);
  const [selected, setSelected] = useState(null); const [conversationId, setConversationId] = useState(null); const [messages, setMessages] = useState([]);
  const [body, setBody] = useState(''); const [busy, setBusy] = useState(false); const [error, setError] = useState('');
  const [otherReadAt, setOtherReadAt] = useState(null); const [otherTyping, setOtherTyping] = useState(false); const [otherTypingAt, setOtherTypingAt] = useState(0);
  const endRef = useRef(null); const presenceRef = useRef(null); const typingTimerRef = useRef(null); const audioRef = useRef(null); const latestMessageIdRef = useRef(null);
  const ownUsername = usernameStatus?.username || session?.user?.user_metadata?.username || 'username';

  const refreshConversations = async () => { try { setConversations(await getMyDirectConversations()); } catch (e) { setError(e.message || 'Could not load conversations.'); } };
  const refreshReadState = async id => { if (!id || !supabase) return; try { const { data, error: readError } = await supabase.rpc('get_conversation_read_state', { p_conversation_id: id }); if (readError) throw readError; setOtherReadAt(data?.[0]?.other_last_read_at || null); } catch (_) {} };
  const refreshMessages = async id => { if (!id) return; try { const data = await getDirectMessages(id); setMessages(current => { const oldLast = current[current.length - 1]?.id; const newLast = data[data.length - 1]?.id; if (newLast && newLast !== oldLast && oldLast && data.some(m => m.id === newLast && m.sender_id !== session.user.id)) playMessagePing(audioRef, true); return data; }); } catch (_) {} };

  useEffect(() => { if (session) refreshConversations(); }, [session]);
  useEffect(() => { if (!open || !query.trim()) { setResults([]); return undefined; } const timer = window.setTimeout(() => searchUsersByUsername(query).then(setResults).catch(e => setError(e.message || 'Search failed.')), 220); return () => window.clearTimeout(timer); }, [open, query]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!active) return;
        setMessages(data); latestMessageIdRef.current = data[data.length - 1]?.id || null;
        await supabase.rpc('mark_conversation_read', { p_conversation_id: conversationId });
        await refreshReadState(conversationId); refreshConversations();
    ...
    const channel = supabase.channel(`direct-chat-${conversationId}`)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `conversation_id=eq.${conversationId}` }, payload => {
        ...
        refreshConversations(); refreshReadState(conversationId);
      })
      ...
      .subscribe(async status => { if (status === 'SUBSCRIBED') await channel.track({ user_id: session.user.id, typing: false, typing_at: 0 }); });

    async function refreshPresence() {
      const state = channel.presenceState();
      const others = Object.values(state).flat().filter(p => p.user_id !== session.user.id);
      const typingEntry = others.find(p => p.typing && Date.now() - Number(p.typing_at || 0) < 5000);
      setOtherTyping(Boolean(typingEntry));
      setOtherTypingAt(typingEntry ? Number(typingEntry.typing_at) : 0);
    }

    // Realtime is primary. This lightweight fallback keeps the thread live even if
    // the browser/network drops a realtime event while the conversation is open.
    const pollTimer = window.setInterval(() => { refreshMessages(conversationId); refreshReadState(conversationId); }, 2000);
    const typingExpiry = window.setInterval(() => { if (otherTypingAt && Date.now() - otherTypingAt >= 5000) { setOtherTyping(false); setOtherTypingAt(0); } }, 1000);
    presenceRef.current = channel;
    return () => { active = false; window.clearInterval(pollTimer); window.clearInterval(typingExpiry); try { channel.untrack(); supabase.removeChannel(channel); } catch (_) {} presenceRef.current = null; setOtherTyping(false); setOtherTypingAt(0); setOtherReadAt(null); };
  }, [conversationId, session?.user?.id]);

  useEffect(() => {
    if (!session || !supabase) return undefined;
    const channel = supabase.channel(`direct-inbox-${session.user.id}`)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages' }, payload => { if (payload.new.sender_id === session.user.id) return; if (payload.new.conversation_id !== conversationId) playMessagePing(audioRef, true); refreshConversations(); })
      .subscribe();
  ...
  useEffect(() => () => { window.clearTimeout(typingTimerRef.current); try { audioRef.current?.close(); } catch (_) {} }, []);
  ...
  const openConversation = async (user, existingId = null) => { setBusy(true); setError(''); try { const id = existingId || await getOrCreateDirectConversation(user.username); setSelected(user); setConversationId(id); setQuery(''); setResults([]); setOpen(true); if (!audioRef.current) audioRef.current = createPingContext(); } catch (e) { setError(e.message || 'Could not open conversation.'); } finally { setBusy(false); } };
  const openMessenger = () => { setOpen(true); setError(''); refreshConversations(); if (!audioRef.current) audioRef.current = createPingContext(); };

  const updateTyping = async value => { const channel = presenceRef.current; if (!channel) return; try { await channel.track({ user_id: session.user.id, typing: value, typing_at: value ? Date.now() : 0 }); } catch (_) {} };
  const handleBodyChange = e => { const value = e.target.value; setBody(value); updateTyping(Boolean(value.trim())); window.clearTimeout(typingTimerRef.current); if (value.trim()) typingTimerRef.current = window.setTimeout(() => updateTyping(false), 5000); };
  const send = async e => { e.preventDefault(); if (!body.trim() || !conversationId || busy) return; setBusy(true); setError(''); try { const msg = await sendDirectMessage(conversationId, body); setMessages(v => v.some(m => m.id === msg.id) ? v : [...v, msg]); latestMessageIdRef.current = msg.id; setBody(''); await updateTyping(false); playMessagePing(audioRef, false); refreshConversations(); } catch (e) { setError(e.message || 'Could not send message.'); } finally { setBusy(false); } };
