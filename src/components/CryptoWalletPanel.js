...
export default function CryptoWalletPanel({ onWalletChanged }) {
  ...
  return <section className="crypto-wallet-panel">
    <div className="crypto-wallet-heading">
      <div>
        <div className="eyebrow">FAV CRYPTO</div>
      ...
      <span className={`crypto-network-badge ${chain?.token_address ? 'ready' : ''}`}>{BASE_SEPOLIA.name}</span>
    </div>
    ...
    {loading ? <div className="crypto-wallet-loading">Loading crypto status…</div> : <>
      ...
      <div className="crypto-wallet-grid">
        ...
        <div className="crypto-wallet-card">
          ...
          {!chain?.token_address ? <div className="crypto-notice"><strong>Testnet token deployment pending</strong><span>Wallet verification is ready, but no FAV can move on-chain until the Base Sepolia contract is deployed and recorded.</span></div> : !chain?.unlock_enabled ? <div className="crypto-notice"><strong>Crypto unlock is disabled</strong><span>The token exists, but Favourit has not enabled the unlock queue yet.</span></div> : <>
    ...
    <small className="crypto-security-note">Never share a seed phrase or private key with Favourit. Wallet linking uses a signature only.</small>
  </section>;
}
