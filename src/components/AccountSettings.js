...
export default function AccountSettings({ session, profile, onProfileChanged, onClose }) {
  ...
  useEffect(() => { getAccountSecurityStatus().then(setSecurity).catch(() => {}); loadBlocked(); }, []);
  useEffect(() => { setDisplayName(profile?.display_name || session?.user?.user_metadata?.display_name || ''); setBio(profile?.bio || ''); }, [profile, session?.user?.id]);

  const saveProfile = async e => { e.preventDefault(); setBusy(true); setError(''); setSaved(''); try { const next = await updateProfileBasics({ displayName, bio }); onProfileChanged?.(next); setSaved('Profile updated.'); } catch (err) { setError(err.message || 'Could not update your profile.'); } finally { setBusy(false); } };
