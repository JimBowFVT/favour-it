...
export default function DirectMessagingV2({ session }) {
  ...
  const refreshPeople = async () => {
    ...
    setConversations(Array.isArray(safe) ? safe : []);
  ...
  const conversationMeta = selectedType === 'community'
    ...
    : currentTyping
      ...
      : friendOnline
        ? 'Online'
        : lastSeen || (selected?.is_message_request ? 'Message request' : isRead ? 'Read' : lastMine ? 'Sent' : '');
    ...
    {open && <div className="dm-overlay" onMouseDown={event => { if (event.target === event.currentTarget) closeMessenger(); }}>
      <section className={`dm-panel ${selected ? 'is-thread' : 'is-inbox'}`}>
        {!selected ? <>
          ...
          {section === 'requests' && <div className="dm-list dm-request-list">
            <div className="dm-request-explainer"><strong>Message requests</strong><span>Messages from people who are not your friends appear here.</span>{requestUnread > 0 && <b>{requestUnread} unread</b>}</div>
            {messageRequests.length ? renderConversationRows(messageRequests) : <div className="dm-empty"><h3>No message requests.</h3><p>New messages from people outside your friends list will appear here.</p></div>}
          </div>}
          ...
          <header className="dm-thread-header">
            ...
            {selectedType === 'direct' ? <button className="dm-thread-identity" onClick={() => openProfile(selected)} type="button">
              <Avatar person={selected} large />
              <span><strong>{selected.display_name || selected.username}</strong><small>@{selected.username}</small><em>{conversationMeta}{friendOnline && <i className="online-dot" />}</em></span>
            </button> : <div className="dm-thread-identity"><span className="dm-community-icon">{GROUP_ICONS[selected.slug] || '◇'}</span><span><strong>{selected.name}</strong><small>Community chat</small><em>{conversationMeta}</em></span></div>}
            <button className="dm-close" onClick={closeMessenger} type="button" aria-label="Close">×</button>
          </header>
          ...
          {replyTo && <div className="dm-reply-bar"><div><span>Replying to @{replyTo.username || selected.username || 'member'}</span><small>{replyTo.body}</small></div><button onClick={() => setReplyTo(null)} type="button">×</button></div>}
