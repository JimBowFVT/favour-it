...
export default function ActivityCenter() {
  ...
  useEffect(() => {
    ...
    let active = true;
    const load = async () => { try { const rows = await getConversationMessages(conversation); if (active) setMessages(rows); await markConversationRead(conversation); await refresh(); } catch (_) {} };
    load();
    const channel = supabase?.channel(`messages-${conversation}`)?.on('postgres_changes', { event:'INSERT', schema:'public', table:'messages', filter:`conversation_id=eq.${conversation}` }, payload => {
  ...
  const handleNotification = async notification => {
    if (!notification.read_at) { try { await markNotificationRead(notification.id); } catch (_) {} setNotifications(current => current.map(n => n.id === notification.id ? {...n,read_at:new Date().toISOString()} : n)); }
    if (notification.conversation_id) setConversation(notification.conversation_id);
  };
  const submit = async event => {
    event.preventDefault(); if (!draft.trim() || !conversation || busy) return; setBusy(true);
    try { const message = await sendMessage(conversation,draft); setMessages(current => current.some(m=>m.id===message.id)?current:[...current,message]); setDraft(''); await refresh(); } catch (_) {} finally { setBusy(false); }
  };

  return <div className="activity-center">
    <button className="activity-trigger" aria-label="Notifications and messages" onClick={()=>{setOpen(v=>!v);setConversation(null);}}>◌{unread>0&&<span className="activity-badge">{unread>9?'9+':unread}</span>}</button>
