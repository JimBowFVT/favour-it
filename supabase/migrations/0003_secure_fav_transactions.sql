...
begin
  ...
  values (v_user, 0, 0) on conflict (user_id) do nothing;
  update public.wallets
  set available_fav = available_fav - v_deal.price_fav,
      held_fav = held_fav + v_deal.price_fav, updated_at = now()
  where user_id = v_user and available_fav >= v_deal.price_fav;
...
-- Platform fees are credited to a dedicated system wallet, not deducted twice
-- from the seller's accounting ledger.
...
begin
  ...
  -- Use a deterministic system UUID for the platform ledger account.
  v_platform := '00000000-0000-0000-0000-000000000001'::uuid;
  insert into public.wallets(user_id, available_fav, held_fav)
  values (v_platform, v_fee, 0)
  on conflict (user_id) do update
    set available_fav = public.wallets.available_fav + excluded.available_fav, updated_at = now();
  ...
  values
    ...
    (v_platform, v_order.id, 'fee', v_fee, 'fee:' || v_order.id::text, jsonb_build_object('platform_fee', true));
  return true;
end;
$$;
...
begin
  ...
  if v_order.status not in ('funded','in_progress','disputed') then raise exception 'order cannot be refunded'; end if;
  update public.wallets
  set held_fav = held_fav - v_order.amount_fav, available_fav = available_fav + v_order.amount_fav, updated_at = now()
  where user_id = v_order.buyer_id and held_fav >= v_order.amount_fav;
  if not found then raise exception 'escrow balance mismatch'; end if;
  ...
  insert into public.ledger_entries(user_id, order_id, entry_type, amount_fav, idempotency_key, metadata)
  values (v_order.buyer_id, v_order.id, 'refund', v_order.amount_fav, 'refund:' || v_order.id::text, jsonb_build_object('reason', 'order_refund'));
  return true;
end;
$$;
