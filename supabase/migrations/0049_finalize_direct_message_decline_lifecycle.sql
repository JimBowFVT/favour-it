...
begin
  ...
  select case when dc.user_one=auth.uid() then dc.user_two else dc.user_one end
    into target
  from public.direct_conversations dc
  ...
  if exists(
    select 1 from public.user_blocks ub
    where (ub.blocker_id=auth.uid() and ub.blocked_id=target)
       or (ub.blocker_id=target and ub.blocked_id=auth.uid())
  ) then raise exception 'Messaging is unavailable because one of you has blocked the other.'; end if;
  ...
  select exists(
    select 1 from public.friendships f
    where f.user_id=auth.uid() and f.friend_id=target
  ) into are_friends;

  select m.sender_id into first_sender
  from public.messages m
  where m.conversation_id=p_conversation_id
  order by m.created_at asc
  limit 1;

  select rs.state into my_request_state
  from public.direct_message_request_states rs
  where rs.conversation_id=p_conversation_id and rs.user_id=auth.uid();

  select rs.state into target_request_state
  from public.direct_message_request_states rs
  where rs.conversation_id=p_conversation_id and rs.user_id=target;
  ...
  if first_sender is not null
     and first_sender<>auth.uid()
     and not are_friends
     and coalesce(my_request_state,'pending')<>'accepted' then
    raise exception 'Accept this message request before replying.';
  end if;

  if first_sender=auth.uid()
     and not are_friends
     and target_request_state='declined' then
    raise exception 'This message request was declined.';
  end if;

  if p_reply_to_message_id is not null and not exists(
    select 1 from public.messages rm
    where rm.id=p_reply_to_message_id
      and rm.conversation_id=p_conversation_id
      and rm.deleted_at is null
  ) then raise exception 'You cannot reply to a deleted or unavailable message.'; end if;

  if exists(
    select 1 from unnest(coalesce(p_asset_ids,'{}'::uuid[])) aid
    where not public.can_use_message_media_asset(aid)
  ) then raise exception 'One or more media items are unavailable.'; end if;
  ...
  if clean_body='' and asset_count=0 and snapshot is null then
    raise exception 'Add text, media, or a deal before sending.';
  end if;
...
begin
  ...
  select dc.conversation_id,auth.uid(),'declined',now()
  from public.direct_conversations dc
  left join public.direct_message_request_states rs
    on rs.conversation_id=dc.conversation_id and rs.user_id=auth.uid()
  where ((dc.user_one=auth.uid() and dc.user_two=p_user_id) or (dc.user_one=p_user_id and dc.user_two=auth.uid()))
    and not exists(select 1 from public.friendships f where f.user_id=auth.uid() and f.friend_id=p_user_id)
    and coalesce(rs.state,'pending') <> 'accepted'
    and (
      select m.sender_id
      from public.messages m
  ...
  where status='pending' and ((sender_id=auth.uid() and receiver_id=p_user_id) or (sender_id=p_user_id and receiver_id=auth.uid()));
  return true;
end;
$$;
