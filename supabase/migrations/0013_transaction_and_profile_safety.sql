...
-- Onboarding rewards: first N full days are zero-value, then normal/premium reward.
create or replace function public.claim_daily_reward()
returns bigint language plpgsql security definer set search_path = public as $$
declare
  v_user uuid := auth.uid(); v_created_at timestamptz; v_days integer; v_usd numeric; v_amount bigint; v_claim_id uuid; v_is_premium boolean := false;
begin
  ...
  if exists (select 1 from public.reward_claims where user_id = v_user and reward_date = current_date) then raise exception 'daily reward already claimed'; end if;
  v_days := greatest(0, floor(extract(epoch from (now() - v_created_at)) / 86400)::integer);
  v_is_premium := exists (select 1 from public.premium_memberships pm where pm.user_id = v_user and pm.active_until > now());
  select case when v_days < onboarding_reward_days then onboarding_daily_reward_usd when v_is_premium then premium_daily_reward_usd else standard_daily_reward_usd end into v_usd from public.economy_config where id = true;
  ...
  insert into public.wallets(user_id, available_fav) values (v_user, v_amount)
  ...
  if v_amount > 0 then
    ...
    values (v_user, case when v_is_premium then 'premium_reward'::public.ledger_entry_type else 'daily_reward'::public.ledger_entry_type end, v_amount,
...
-- Buyer settlement is allowed only after seller delivery.
create or replace function public.release_order(p_order_id uuid)
returns boolean language plpgsql security definer set search_path = public as $$
declare
  v_user uuid := auth.uid(); v_order public.orders%rowtype; v_payout bigint; v_fee bigint;
begin
  if v_user is null then raise exception 'not authenticated'; end if;
  ...
  v_fee := v_order.fee_fav; v_payout := v_order.amount_fav - v_fee;
  ...
  insert into public.wallets(user_id, available_fav, held_fav) values (v_order.seller_id, v_payout, 0)
    on conflict (user_id) do update set available_fav = public.wallets.available_fav + excluded.available_fav, updated_at = now();
  insert into public.platform_accounts(id, available_fav) values (true, v_fee)
    on conflict (id) do update set available_fav = public.platform_accounts.available_fav + excluded.available_fav, updated_at = now();
  update public.escrow_transactions set status = 'released', released_at = now() where order_id = v_order.id and status = 'held';
  ...
  insert into public.ledger_entries(user_id, order_id, entry_type, amount_fav, idempotency_key, metadata) values
    (v_order.buyer_id, v_order.id, 'escrow_release', -v_order.amount_fav, 'escrow-release-buyer:' || v_order.id::text, jsonb_build_object('completed', true)),
    (v_order.seller_id, v_order.id, 'sale', v_payout, 'sale:' || v_order.id::text, jsonb_build_object('gross_fav', v_order.amount_fav, 'fee_fav', v_fee));
  return true;
end;
$$;
...
-- Dispute resolution must settle escrow; it can no longer close a dispute with funds held.
create or replace function public.resolve_dispute(p_dispute_id uuid, p_resolution text, p_note text default '')
returns void language plpgsql security definer set search_path = public as $$
declare
  v_user uuid := auth.uid(); v_dispute public.disputes%rowtype; v_order public.orders%rowtype; v_net bigint;
begin
  if v_user is null then raise exception 'not authenticated'; end if;
  ...
  if p_resolution = 'refund_buyer' then
    ...
    update public.escrow_transactions set status = 'refunded', released_at = now() where order_id = v_order.id and status = 'held';
    if not found then raise exception 'escrow transaction is not held'; end if;
    ...
    update public.wallets set held_fav = held_fav - v_order.amount_fav, updated_at = now()
      where user_id = v_order.buyer_id and held_fav >= v_order.amount_fav;
    if not found then raise exception 'buyer escrow balance unavailable'; end if;
    insert into public.wallets(user_id, available_fav, held_fav) values (v_order.seller_id, v_net, 0)
      on conflict (user_id) do update set available_fav = public.wallets.available_fav + excluded.available_fav, updated_at = now();
    insert into public.platform_accounts(id, available_fav) values (true, v_order.fee_fav)
      on conflict (id) do update set available_fav = public.platform_accounts.available_fav + excluded.available_fav, updated_at = now();
    update public.escrow_transactions set status = 'released', released_at = now() where order_id = v_order.id and status = 'held';
    if not found then raise exception 'escrow transaction is not held'; end if;
    insert into public.ledger_entries(user_id, order_id, entry_type, amount_fav, idempotency_key, metadata)
      values (v_order.seller_id, v_order.id, 'sale', v_net, 'dispute-release-seller:' || v_order.id, jsonb_build_object('dispute_id', p_dispute_id, 'fee_fav', v_order.fee_fav));
    update public.orders set status = 'completed', completed_at = now(), updated_at = now() where id = v_order.id;
  end if;
  update public.disputes set status = 'resolved', resolution = p_resolution, resolved_by = v_user, resolution_note = left(coalesce(p_note,''), 5000), resolved_at = now() where id = p_dispute_id;
end;
$$;
...
-- A normal user cannot self-verify. Trusted workflows can still update another user's profile.
...
begin
  if new.is_verified is distinct from old.is_verified and auth.uid() = old.id then
    raise exception 'verification status can only be changed by trusted workflows';
  end if;
  return new;
end;
$$;
drop trigger if exists protect_profile_trusted_fields on public.profiles;
create trigger protect_profile_trusted_fields before update on public.profiles for each row execute function public.protect_profile_trusted_fields();

-- Review identity cannot be retargeted after creation.
create or replace function public.protect_review_identity()
returns trigger language plpgsql as $$
begin
  if new.order_id is distinct from old.order_id or new.reviewer_id is distinct from old.reviewer_id or new.seller_id is distinct from old.seller_id then
...
create trigger protect_review_identity before update on public.reviews for each row execute function public.protect_review_identity();

-- Reviews can only target the buyer/seller pair of a completed order.
...
create trigger validate_review_target before insert on public.reviews for each row execute function public.validate_review_target();
